...
async function registerController(req,res){
    ...
    const hash= await bcrypt.hash(password, 10); // the number(salt) is simply the number of layers of hashing we want to apply to our variable.
...
async function loginController(req,res){
    ...
    // const hash= crypto.createHash("sha256").update(password).digest("hex");
     ...
     const isPasswordCorrect= await bcrypt.compare(password, user.password);    //hash==user.password  using bcrypt reduces our lines of codes and also improves the efficiency.
